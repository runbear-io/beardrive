...
| `bdrive lesson <text>` | Record a correction for every teammate's agent: appends one line (`- <text> — <you>, <date>`) to `lessons/<device-id>.md` in the project and syncs it. On its next turn, each teammate's agent is told the new line once by the sync hook (capped at 10 per turn). One file per device, so two people recording at once never conflict; edit the file to change or remove a lesson. `--folder` picks the project. Refuses, writing nothing, when `lessons/` is ignored, outside the sync scope, or not writable for you. Offline, it records and pushes on the next cycle |
...
| `bdrive restore <file> [version]` | Put an earlier version of a file back, as a new change (`--list` shows the versions, exactly one marked `*` as current; no version = the previous one). Nothing is erased and it syncs everywhere like any edit. Re-reads the file after the sync it ends with and exits non-zero if the bytes on disk are not the version you asked for, so the success line never names a version the file does not hold. To un-create a file a run *created*, use **undo — remove file** on that row in the hub's History view — or **undo this run** in the run card's header to put back every file that run touched at once |
