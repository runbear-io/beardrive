...
| `bdrive status [folder]` | Projects, daemon state, and two separate change counts: `pending` (journalled, not yet pushed) and `local` (on disk, not yet scanned — what a stopped daemon leaves invisible). Pure local read: no ops, no journal writes, no network |
