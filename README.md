...
| `bdrive autostart [install\|uninstall]` | Show, add, or remove the login registration that runs `bdrive resume` after a reboot (macOS: a user LaunchAgent). `bdrive init` installs it; `--no-autostart` skips it |
