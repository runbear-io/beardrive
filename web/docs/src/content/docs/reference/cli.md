...
| `bdrive lesson <text>` | Record a correction every teammate's agent is told on its next turn. Appends one line to `lessons/<device-id>.md` and syncs it; one file per device, so simultaneous lessons never conflict. `--folder` picks the project. Refuses, writing nothing, when `lessons/` would not reach the hub (ignored, out of scope, or read-only for you). See [Shared agent memory](/guides/shared-agent-memory/#correct-one-agent-every-agent-learns) |
