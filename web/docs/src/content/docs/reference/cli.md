...
| `bdrive url [path]` | Internal hub link for a file or folder — sign-in and membership required. Opens the viewer in a browser; fetched with an `Accept` that names no `text/html` the same URL returns the file itself plus `X-Bdrive-Provenance` (see [Artifacts & links](/guides/agent-artifacts/)). `--sync` pushes first, and warns on stderr if the hub refused that push; no argument gives the project home. Computed locally |
