...
| `bdrive autostart [install\|uninstall]` | Show, add, or remove the login registration that runs `bdrive resume` after a reboot: a user LaunchAgent on macOS, a systemd user unit on Linux (needs systemd). Windows falls back to running `bdrive resume` yourself. `bdrive init` installs it; `--no-autostart` skips it |
