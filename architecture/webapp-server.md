...
classDiagram
    ...
    class Server {
        ...
        +headBlob(ctx, volume, path) (sha, ok)
        -pathLocks sync.Map (one mutex per project+path)
    }
    class recordingMux {
    ...
    class ConditionalPutter {
        <<interface>>
        +GetVersioned(ctx, key) body, Version
        +PutIf(ctx, key, r, size, expect) Version
    }
    note for ConditionalPutter "internal/remote — optional, same mold again: only gcsBackend implements it, over object generations. It exists because a journal append is a READ-MODIFY-WRITE and Put is last-writer-wins: two hub processes both read one journal, both append their own ops, and whichever writes second silently erases the other's with no error anywhere, because both writes succeeded. That is the lost update the &quot;each device writes only its own journal&quot; invariant prevents BETWEEN devices, reappearing inside ONE device id as soon as the hub runs twice. The precondition has to be evaluated by the STORE because that is the only party the two processes share — nothing either of them holds can serialise the other. Version is OPAQUE (a generation, an ETag, a fingerprint share nothing but equality) and VersionAbsent means &quot;create, and fail if somebody beat me&quot;. appendOps retries a bounded five times: the ops are already stamped, so a retry appends onto what is there NOW rather than duplicating. upmu is kept — it stops N goroutines in one process taking the slow path, which is a different job. S3 and file:// deliberately do NOT implement it: conditional PutObject is unevenly supported, and a cross-process compare-and-swap over a rename would be a worse lie than declining the capability"
    ...
    class ydocs {
        &lt;&lt;reearth/ygo, embedded&gt;&gt;
        +ServeHTTP websocket, behind proj(PermRead)
        +Authorize per connection, ReadOnly for a reader
        +OnLoadDocument seed from the file
    ...
    note for reservations "The seam between a presigned URL and the plan it spends. CheckWrite alone answered per request, so N concurrent signs each passed against the same free space and the org wrote N times its quota. Every write door now charges billableBytes + reservedBytes(org), reserveIfFits is the compare-and-set, and a signed-but-unspent grant expires. billableBytes is zero for a blob key the store ALREADY holds: blobs are content-addressed, so rewriting one is byte-identical and grows storage by nothing. Co-editing is where that stops being an edge case — sharedfile.ts restarts every peer's idle timer on any change by anyone, so all N editors save the same converged text at once and every writer after the first is a no-op put. Charging them all billed a room of seven 7x for one blob, against a cloud counter that only ever climbs (deletes never decrement it; only the reconciler corrects it), which walked orgs into 'storage limit reached' without storing anything. Journals are exempt — they are appended to, so a rewrite really is bigger. reconcileGrants asks the backend whether the blob actually landed and either RecordUsage-s it or gives the space back — the reservation is a hold, never the accounting"
