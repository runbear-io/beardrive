...
classDiagram
    ...
    class Commands {
        ...
        restore url share capture export import
    ...
    note for Commands "capture pipes stdin into the mount root's inbox/&lt;timestamp&gt;.md and prints the path; --share runs one Cycle then mintShare (extracted from share, which now only prints what it returns). The scope gate is the whole command: it applies syncer.Filter.SkipUp with acceptedRules(proj.ID) installed and REFUSES BEFORE WRITING when inbox/ is excluded — a bare LoadFilter reads a teammate's pulled !inbox/ widening as &quot;syncs&quot; while the cycle silently refuses to upload it, and a file that lands on disk and never syncs is the exact failure the command exists to fix. A paused or unenrolled project only warns (it syncs the moment someone re-inits); --share refuses there, having no cycle to run"
