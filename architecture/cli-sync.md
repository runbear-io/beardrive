...
classDiagram
    ...
    note for Session "internal/syncer — re-derive the clock → scan → commit local ops → pull peer journals → adopt on join → re-assert withdrawn ops → preserve conflicts → refresh rules → prune → materialize → push blobs then own journal"
    note for Session "The cycle OPENS by folding every op already in this volume's journal dir through absorbLamport. Nothing else re-derives st.Lamport — the pull absorbs only its NEW tail, and a settled peer journal yields none — so a device whose sync.json fell below the journals beside it stayed there forever, and since Less orders on lamport before time, every op it wrote lost replay: pushed for real, then overwritten by materialize in the same cycle (BEA-196). Derived AFTER `joining` is read, or a device whose first cycle crashed stops being a joiner and adoption changes"
    ...
    note for daemon "The tick now selects on a THIRD arm: remote.Watcher's channel, when the backend is a hub that implements it. A peer's push clears lastRemote so the next cycle is a remote one, turning ~10s of latency into ~1s. The stream is torn down wherever the backend is dropped (token change, offline) since its connection carries the old credential to the old host, and a failed or ended dial backs off watchRetry (1m) rather than retrying per tick — an older hub 404s here forever and must not become a log line every few seconds. NO LONGER strictly an accelerator: a live stream now also SETS the remote cadence, which drops to remoteInterval x watchedPollFactor (5m at the default) and snaps back to remoteInterval the moment watch goes nil — a failed dial, a token change, an offline cycle or the hub&apos;s own streamMaxAge all do that, so losing the stream restores the tight cadence on the very next tick. The tick is still the guarantee: a stream can die silently and nothing trusts it further than that factor. This is the largest single item in docs/hub-load-prd.md — an idle device on one project cost 17,280 requests a day, 9.4% of the production hub&apos;s entire daily traffic, to be told nothing by a poll while an open stream told it the same nothing for free. A hub with no stream syncs exactly as it always did"
...
classDiagram
    ...
    class Commands {
        init login logout
        sync stop scope grep stale forget status log lesson
    ...
    note for Commands "lesson appends one line to lessons/&lt;device-id&gt;.md — one file per DEVICE, so two people recording at once never share a writer and never produce a conflict copy — after refusing anything that would not reach the hub (SkipUp, persisted ReadOnly/Denied/Access), then runs one cycle. sync --hook injects teammates' NEW lesson lines from the paths it already drained (never a second drain, never a walk): lines are hashed against lessons-seen.json in the volume store, a file must open with the lesson header, cap 10 per turn, and the seen set is saved only after the context is emitted"
