...
classDiagram
    ...
    note for hooks "usePresence announces the path you are on and renders the roster the hub pushes back — the roster ARRIVES on useProjectEvents' stream (onPresence), not on the POST, which is used only for first paint so a tab opening into a quiet project still sees who is there. One EventSource carries both frame types: presence invalidates nothing. There is NO heartbeat: it was 10s per TAB — unlike the stream, which is leader-elected per browser — and ~19% of every request the hub served, sent by people who were reading (docs/hub-load-prd.md). Liveness is the stream itself now: the hub keys presence to that connection and drops you when your last one closes, so the uplink fires only on arrive, navigate and leave. Leaving is a SEPARATE effect that does not depend on path — folded into the other one it sent a leave racing the arrive behind it, dropping you off your own teammates' rosters. Every failure is swallowed: presence is decoration and must never surface an error"
    ...
    note for lib "openSharedFile arms the idle save on MY changes only: Y.Text.observe gets the transaction, and CollabDoc.isRemote tests its origin against the y-websocket provider. It used to arm on ANY change including a peer&apos;s — &quot;whoever stops typing last writes the file&quot; — so a room of N editors each PUT the identical full body 700ms after the last keystroke by anyone: N writes of one text and N History versions per quiet period, which is the too-many-versions report. Nothing is lost by dropping to one writer: every copy is identical by construction and ycollab.go still snapshots on last peer. Seeding is a remote change too, so the document arriving no longer marks the buffer dirty and schedules a save of bytes the file already has. rebase(sha) exists because of what that exposed: `base`, the If-Match every save carries, only ever moved on our OWN write, so a co-editor&apos;s save moved the head and everyone else was refused for a conflict that did not exist — parking work in a conflict copy of the file against itself. The sha is adopted only where merge ACCEPTS the content, never on blocked, or If-Match stops being a guard"
    ...
    class lib {
        ...
        +collab.ts CollabDoc peerCount isRemote (y-websocket to the hub-held document)
        ...
        +sharedfile.ts rebase (the head sha moved because a co-editor saved)
