...
| # | Boundary | State after round 13 | Attacks that must be tried |
|---|----------|---------------------|----------------------------|
...
Three hackers landed (a fourth, agent-onboarding, folded in separately). 39
failing assertions across 17 failing test functions came in; all 17 are green,
`go build` / `go vet` / `go test ./...` are clean **with and without**
`BDRIVE_TEST_POSTGRES`, `-race -timeout 30m` on webapp is clean (1270s, no data
races), and Playwright is **127/127** run serially.

**One check was NOT completed and is not claimed:** the Linux container run.
Docker Desktop's bind-mounted build did not finish in 50 minutes and was
abandoned. What ran instead is `GOOS=linux go build ./... && GOOS=linux go vet
./...`, which is clean — that catches build tags and type errors, not runtime
behaviour. The round's changes contain no OS-specific code (no syscalls, no
path semantics, no filesystem assumptions; `internal/store`'s flock,
`internal/daemon`'s kill and `internal/autostart` are untouched), so the
exposure is judged low — but it is an unrun check, not a passed one, and round
14 should run it.

All three hackers stated the commit they ran at and whether `sec_*.go` were
present. **The process fix from round 12's instance-3 incident worked — keep
requiring it.**
...
- **Condition 2 — dry rounds:** round 13 was the opposite of dry. Three hackers
  produced 17 failing test functions including a hub-wide authorization wall
  (`OrgDB`) and a permanent-lockout bug. The counter stands at **zero
...
## Coverage gaps after round 12 — round 13's targets
...
## Coverage gaps after round 11 — round 12's targets
...
## Coverage gaps after round 9 — round 10's targets
