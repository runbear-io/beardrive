...
| Undo a change to a file | `bdrive restore <file> [<version>]` — writes an earlier version back as a NEW change (never rewrites history); no version = the previous one, `--list` shows them. Also in the hub's History view, which additionally has **undo — remove file** on a file a run created (see below) |
...
**Restore puts content back; it does not delete.** To un-create a file a run *created*, open that run in the hub's History view and use the row's **undo — remove file** button: it confirms first (the file leaves every synced device), journals a delete, and the DELETED row it leaves behind restores the file. From the CLI, delete the file normally and let the next sync carry that.
